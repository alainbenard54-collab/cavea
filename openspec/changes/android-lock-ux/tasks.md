...
- [ ] 4.4 Android — exit sans libérer : passer en écriture, quitter l'app (swipe), relancer → vérifier dialog "Session précédente" avec confirmation avant exécution [TEST MANUEL]
...
- [ ] 4.7 Android — crash recovery confirmations : vérifier dialog de confirmation avant "Envoyer mes données locales" et avant "Repartir depuis Drive" [TEST MANUEL]
- [ ] 4.8 Android — "Retour lecture seule" : en mode écriture, taper "Retour lecture seule", lire confirmation, confirmer → snackbar, retour lecture seule, lock absent sur Drive [TEST MANUEL]
