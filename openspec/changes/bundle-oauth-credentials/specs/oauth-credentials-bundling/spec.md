...
- **THEN** l'app lit l'App Key depuis l'asset `assets/secrets/dropbox_app_key.txt` et déclenche le flow PKCE sans demander de saisie à l'utilisateur
...
#### Scenario: Android sans asset dropbox_app_key
- **WHEN** l'APK est buildé sans `assets/secrets/dropbox_app_key.txt` et que l'utilisateur tente d'activer Dropbox Mode 2
