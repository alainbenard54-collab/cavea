...
- **WHEN** la vue racine de l'arbre d'emplacements (`LocationTreeScreen`, sans navigation) est rendue avec un jeu de données de test fixe
- **THEN** le rendu correspond pixel pour pixel à l'image de référence versionnée dans `test/golden/emplacements/`

#### Scenario: Bascule table/liste d'un emplacement — explicitement hors périmètre
- **WHEN** un développeur envisage un golden test sur la bascule table/liste ≥640px affichée après navigation dans un emplacement
- **THEN** ce cas n'est pas couvert : ce rendu réutilise les mêmes composants (`StockTable`/liste de bouteilles) déjà couverts par le golden Stock — un test dédié serait redondant
