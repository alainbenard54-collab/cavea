...
`DropboxStorageAdapter` SHALL authentifier l'utilisateur sur Android via OAuth 2.0 PKCE avec redirect localhost (même flow que desktop — HttpServer sur loopback accessible depuis Chrome Android). Le refresh_token SHALL être stocké dans `flutter_secure_storage` sous `dropbox_refresh_token`.
...
- **THEN** l'app ouvre le browser via `url_launcher` sur l'URL d'autorisation Dropbox, le callback localhost est capturé par l'HttpServer, échange le code contre des tokens, stocke le refresh_token
