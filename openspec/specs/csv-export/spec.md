...
- **THEN** l'écran est accessible, la section Export est active, la section Import affiche uniquement un message "Import indisponible — la cave est verrouillée par un autre appareil"
...
Sur Android, l'écran SHALL proposer deux boutons d'export : **"Enregistrer"** (utilise `FilePicker.platform.saveFile(bytes: ...)` — Android Storage Access Framework) et **"Partager…"** (utilise `share_plus` `ShareParams` — feuille de partage système). Le CSV SHALL être écrit dans un fichier temporaire avant d'être partagé via share_plus.

#### Scenario: Enregistrer via FilePicker
- **WHEN** l'utilisateur appuie sur "Enregistrer" sur Android
- **THEN** le dialog Android "Enregistrer sous" s'ouvre ; FilePicker écrit le fichier directement via les bytes passés

#### Scenario: Partager via share_plus
- **WHEN** l'utilisateur appuie sur "Partager…"
- **THEN** la feuille de partage système Android s'ouvre avec le fichier CSV (Drive, e-mail, Bluetooth, etc.)
...
- **THEN** le champ nom du fichier est pré-rempli avec la date du jour au format `cave_YYYY-MM-DD.csv`
