...
| Revue de code | skill `/code-review` | Avant tout commit qui touche du comportement ou une surface sensible (logique métier, schéma de données, appel externe, auth/sécurité). Dispense possible uniquement pour typo/doc/formatage/renommage sans impact comportemental — dans le doute, faire la revue. **Invocation strictement manuelle** : ce skill refuse tout appel par le modèle (`disable-model-invocation`) — au déclencheur, demander à l'utilisateur de taper `/code-review` lui-même, ne jamais tenter de l'invoquer via l'outil Skill même après accord explicite. |
...
- Bouton **"Revenir en local"** (`_deactivate`) — désactivé sur Android : signOut, `storageMode = 'local'`, conserve `dbPath`, snackbar "Mode local activé" — pas de wizard
- Bouton **"Changer de fournisseur"** (`_changeProvider`) — disponible sur toutes les plateformes : signOut, `configService.reset()`, relance le wizard (`context.go('/setup')`) pour reconfigurer le mode et le fournisseur

**5. À propos**
- Version, licence, lien vers les licences des dépendances
- Bouton "Documentation" → ouvre `https://cavea.abapps.fr/{fr|en}/` dans le navigateur selon la langue courante de l'app (`url_launcher`, `LaunchMode.externalApplication`)
